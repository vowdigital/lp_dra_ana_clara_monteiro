...
function resultImage(asset, alt) {
  const source = asset.webp && asset.webpSmall
    ? `<source srcset="${asset.webpSmall} 360w, ${asset.webp} ${asset.width}w" sizes="(max-width: 560px) 45vw, 500px" type="image/webp" />`
    : '';
  return `<picture>${source}<img src="${asset.jpg}" alt="${alt}" width="${asset.width}" height="${asset.height}" loading="lazy" decoding="async" /></picture>`;
}

export function renderResults(container, items) {
  container.innerHTML = `${items.length > 1 ? `<div class="results-selector" role="tablist" aria-label="Selecionar caso">${items.map((item, index) => `<button type="button" role="tab" id="result-tab-${item.id}" aria-selected="${index === 0}" tabindex="${index === 0 ? '0' : '-1'}" aria-controls="result-panel-${item.id}" data-result-tab="${item.id}">${item.procedure || `Caso ${item.id}`}</button>`).join('')}</div>` : ''}<div class="results-gallery">${items.map((item, index) => { const context = [item.procedure, item.context].filter(Boolean).join(' · '); return `<figure class="result-case" id="result-panel-${item.id}" role="tabpanel" aria-labelledby="result-tab-${item.id}" data-result-panel="${item.id}" aria-hidden="${index !== 0}"${index ? ' data-result-hidden' : ''}><div class="result-comparison"><figure class="result-shot">${resultImage(item.before, `Caso ${item.id}, antes do tratamento`)}<figcaption>Antes</figcaption></figure><figure class="result-shot">${resultImage(item.after, `Caso ${item.id}, após o tratamento`)}<figcaption>Depois</figcaption></figure></div>${context ? `<p class="result-context">${context}</p>` : ''}<figcaption class="result-disclaimer">Caso individual. Resultados variam; a indicação depende de avaliação médica individualizada.</figcaption></figure>`; }).join('')}</div>`;
}
